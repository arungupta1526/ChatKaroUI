...
public class MarkdownParser {
    ...
    private static void applyInlineSpans(SpannableStringBuilder ssb,
        ...
        applyPattern(ssb, P_BOLD_STAR,  (s, start, end, groups) ->
            wrapSpan(ssb, start, end, groups[0], new StyleSpan(Typeface.BOLD)));
        applyPattern(ssb, P_BOLD_UNDER, (s, start, end, groups) ->
            wrapSpan(ssb, start, end, groups[0], new StyleSpan(Typeface.BOLD)));
        applyPattern(ssb, P_ITALIC_STAR,(s, start, end, groups) ->
            wrapSpan(ssb, start, end, groups[0], new StyleSpan(Typeface.ITALIC)));
        applyPattern(ssb, P_ITALIC_UNDER,(s, start, end, groups) ->
            wrapSpan(ssb, start, end, groups[0], new StyleSpan(Typeface.ITALIC)));
        applyPattern(ssb, P_STRIKE,     (s, start, end, groups) ->
            wrapSpan(ssb, start, end, groups[0], new StrikethroughSpan()));
        ...
        java.util.regex.Matcher mIC = java.util.regex.Pattern.compile("\u0001CODE\u0001(\\d+)\u0001").matcher(str);
        ...
        for (int j = icMatches.size() - 1; j >= 0; j--) {
            ...
            ssb.setSpan(new TypefaceSpan("monospace"), range[0], range[0] + code.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            ssb.setSpan(new BackgroundColorSpan(0xFFEEEEEE), range[0], range[0] + code.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

        str = ssb.toString();
        java.util.regex.Matcher mCB = java.util.regex.Pattern.compile(java.util.regex.Pattern.quote(CODE_PLACEHOLDER_PREFIX) + "(\\d+)\u0000").matcher(str);
    ...
    private static void applyPattern(SpannableStringBuilder ssb,
        ...
        while (m.find()) {
            ...
            for (int g = 0; g < m.groupCount(); g++) groups[g] = m.group(g + 1);
    ...
    private static void replaceHeadingMarker(SpannableStringBuilder ssb,
                                              ...
                                              float relativeSize) {
        String str = ssb.toString();
        java.util.regex.Pattern p = java.util.regex.Pattern.compile(java.util.regex.Pattern.quote(startMarker) + "(.*?)" + java.util.regex.Pattern.quote(endMarker));
